...
const resultsMap: Record<ResultKey, ResultContent> = {
  ...
  system_wide: {
    label: "System-Wide Strain",
    headline:
      "This doesn’t look like one isolated issue. Your system is under pressure in more than one place.",
    snapshot:
      "Your result does not point to a single clear weak spot. Instead, it suggests the strain is spread across the system, which is why everything may feel heavier, slower, or harder to trust.",
    looksLike: [
      "Multiple parts of marketing feel harder than they should",
      "Reporting, tools, and execution all carry some friction",
      "No single fix feels like it would solve the whole problem",
      "You may be compensating manually in several places",
      "The issue feels broad, not isolated"
    ],
    broken: [
      "The strain is spread across the system rather than concentrated in one place",
      "Different parts of marketing are carrying overlapping friction"
    ],
    missing: [
      "A clearer overall structure",
      "A way to reduce system-wide complexity",
      "Prioritization so you are not trying to fix everything at once"
    ],
    startHere: [
      "Look at the 2 or 3 areas that feel heaviest most often",
      "Choose the one that creates the most downstream friction",
      "Start there rather than trying to fix everything at once"
    ],
    nextSteps: [
      "Reduce friction in the area that affects the most other work",
      "Stabilize one part of the system before moving to the next",
      "Use structure and prioritization, not more effort"
    ],
    ignore: ["Trying to fix every issue at once", "Adding more tools", "Starting over completely"],
    reassurance:
      "This is still a useful result. It means the problem is broader than one category, so the right move is prioritization, not panic."
  }
};
...
const LANDING_PAGE_BASE_URL = "https://glowsparkdigital.com/fix-your-marketing-system";
...
export default function ResultsPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const lowestCount = averages.filter(
        (item) => item.average === lowestAverage
      ).length;
    ...
    <main className="min-h-screen bg-[#f3efef] px-6 py-12">
      <div className="mx-auto max-w-3xl space-y-6">
        ...
        <section className="rounded-2xl border border-[#c9bcad] bg-white p-6">
          ...
          <p className="mb-6 leading-7 text-[#2a1f1c]">
            You’ll see where your marketing system is making things harder than
            it should be, and where to start.
          </p>
